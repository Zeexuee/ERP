...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
            ...
            <p class="text-xs text-slate-500 font-medium mt-1">Kelola tagihan, pantau piutang belum lunas, dan ekspor/impor data faktur ke Excel.</p>
        </div>
        <div class="flex flex-wrap items-center gap-2.5 self-start">
            <a href="{{ route('invoices.export.excel') }}" class="px-4 py-2 rounded-full btn-subtle text-xs font-bold transition">
                Export Excel
            </a>
            <button type="button" onclick="openImportInvoiceModal()" class="px-4 py-2 rounded-full btn-subtle text-xs font-bold transition">
                Import Excel / CSV
            </button>
        </div>
    </div>
...
<script>
    function openImportInvoiceModal() {
        document.getElementById('importInvoiceModal').classList.remove('hidden');
    }

    function closeImportInvoiceModal() {
        document.getElementById('importInvoiceModal').classList.add('hidden');
    }
</script>
@endsection

@push('modals')
<!-- Import Invoice Liquid Glass Modal (Bright & Luminous) -->
<div id="importInvoiceModal" class="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md hidden p-4 overflow-y-auto">
    <div class="bg-white/95 backdrop-blur-3xl max-w-lg w-full p-6 md:p-8 rounded-3xl border border-white/90 shadow-[0_25px_60px_-15px_rgba(0,0,0,0.15)] ring-1 ring-slate-900/5 relative my-8">
        ...
        <div class="flex items-start justify-between pb-4 mb-5 border-b border-slate-100">
            <div>
                <!--<span class="text-[10px] font-extrabold uppercase tracking-widest text-slate-400 block">Billing Integration</span>
                <h3 class="text-xl font-extrabold text-slate-900 tracking-tight">Import Faktur Invoice dari Excel / CSV</h3>
                <p class="text-xs text-slate-500 font-medium mt-0.5">Unggah file spreadsheet untuk mencatat atau memperbarui faktur tagihan secara massal.</p>
            ...
            <button type="button" onclick="closeImportInvoiceModal()" class="w-8 h-8 rounded-full bg-slate-100 hover:bg-slate-200 text-slate-500 hover:text-slate-800 flex items-center justify-center text-xs font-bold transition">
                ✕
            </button>
        </div>

        <form action="{{ route('invoices.import.excel') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            <div>
            ...
            <div class="p-4 rounded-2xl bg-slate-50/90 border border-slate-200/80 shadow-sm space-y-2">
                <div class="flex items-center justify-between">
                    <span class="text-xs font-bold text-slate-900">Format Kolom Faktur</span>
                    <span class="text-[10px] font-bold text-slate-500 font-mono">CSV / XLSX</span>
                </div>
                <p class="text-xs text-slate-600 font-medium leading-relaxed">
                    Struktur kolom: <code class="text-[11px] font-mono bg-white px-1.5 py-0.5 rounded border border-slate-200 text-slate-800">nomor_sales_order, total_tagihan, jatuh_tempo_yyyy_mm_dd, status_faktur</code>
                </p>
                <div class="pt-1">
                    <a href="{{ route('invoices.template.excel') }}" class="inline-flex items-center gap-1.5 text-xs font-bold text-slate-900 bg-white hover:bg-slate-100 px-3.5 py-1.5 rounded-xl border border-slate-200 shadow-sm transition">
            ...
            <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
                <button type="button" onclick="closeImportInvoiceModal()" class="px-5 py-2.5 rounded-full btn-subtle text-xs font-bold transition">Batal</button>
